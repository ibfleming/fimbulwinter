#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# Fimbulwinter — r2modman Profile Exporter
#
...
#   Default output: dist/Fimbulwinter-v<version>-profile.r2z
#
...
if not out:
    out = os.path.join(repo, "dist", f"Fimbulwinter-v{version}-profile.r2z")
os.makedirs(os.path.dirname(os.path.abspath(out)) or ".", exist_ok=True)

lines = [f"profileName: Fimbulwinter-v{version}", "mods:"]
